...
class LoginMessage(BSMessageReader):
    ...
    def decode(self):
        ...
        def process(self):
            if self.player.ban:
                OutOfSyncMessage(self.client, self.player, f"Ваш аккаунт заблокирован!\nПричина блокировки: {self.player.reason}").send()
    ...
    def process(self):
        if self.major != 29:
            ...
            LoginFailedMessage(self.client, self.player, "Ваша версия клиента устарела\nобновитесь до новой!").send()

        if self.player.maintenance:
            LoginFailedMessage(self.client, self.player, "Идет технический перерыв\nпожалуйста дождитесь его завершения!").send()
